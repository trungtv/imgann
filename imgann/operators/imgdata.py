...
class ImgData:
        ...
        if type(folders) == str:
            ...
            if len(folders) == 1:
                ...
                for folder in folders:
                    files = ImgData.ext_files(os.path.join(os.path.abspath(dataset_path), folder))
                    imgFiles = ImgData.__filterImg(files)
                    if files:
                        data_list.extend(
                            ImgData.list_creator(os.path.join(os.path.abspath(dataset_path), folder), folder, imgFiles))
                    else:
                        continue
    ...
    def list_creator(root: str, folder: str, files: list):
        ...
        tol_list = []
        for file in files:
            tol_list.append((file, folder, os.path.join(root, file)))
        return tol_list
